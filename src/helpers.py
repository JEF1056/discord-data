...
def clean(text, author=None, author_id=False):
    ...
    if author and author_id:
        ...
        if author == "Deleted User" or " " in author:
            author = gen_name(author_id)
        text = f"{author.strip()}: {text.strip()}"
    ...
    if text.startswith(": ") and author != None: text=gen_name(author)+text

    for badword in badwords:
        if badword.lower().strip() in text.lower(): return None
